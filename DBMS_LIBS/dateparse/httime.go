...
// GetTimestamp looks
// through the event map for
// something that looks like
// a
// timestamp.
//
// It will guess at the key
// name or use the specified
// one if it is not an empty
// string.  If unable to
// parse the timestamp, it
// will return the current
// time.
// The time field will be
// deleted from the map if
// found.
...
func NginxTimeStamp(timeStr string) (time.Time, error) {
	var (
		ts time.Time
	)
	ts = tryTimeFormats(timeStr, commonLogFormatTimeLayout)
	if ts.IsZero() {
		//ts = Now()
		return ts, fmt.Errorf("time parse err")
	}
	//logrus.Debugf("%s to time.Time %s", timeStr, ts.String())
	return ts, nil
}

func NginxiIsoTimeStamp(timeStr string) (time.Time, error) {
	var (
		ts time.Time
	)
	ts = tryTimeFormats(timeStr, iso8601TimeLayout)
	if ts.IsZero() {
		ts = Now()
		return ts, fmt.Errorf("time parse err")
	}
